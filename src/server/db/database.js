...
const dbPath = process.env.DATABASE_URL || path.join(__dirname, '../../../forge.db');
...
export function initSchema() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      role TEXT NOT NULL CHECK(role IN ('OPERATIVE', 'VANGUARD', 'STUDENT_LEADER', 'TEACHER', 'DEV_STEALTH')),
      tag TEXT,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP
    );
  `);

  // 2. Student Leader Rotations Table
  db.exec(`
  ...
  // 4. Tasks & Challenges Table (task_type: TEAM_TASK vs CHALLENGE)
  db.exec(`
  ...
  // Migration helper: Ensure task_type and mode columns exist on existing databases
  try {
    const tableInfo = db.prepare('PRAGMA table_info(tasks)').all();
    const hasTaskType = tableInfo.some(col => col.name === 'task_type');
    if (!hasTaskType) {
      db.exec(`ALTER TABLE tasks ADD COLUMN task_type TEXT NOT NULL DEFAULT 'TEAM_TASK';`);
      db.exec(`ALTER TABLE tasks ADD COLUMN mode TEXT NOT NULL DEFAULT 'CHOICE';`);
    }
  } catch (err) {
    // Ignore migration error if columns exist
  }

  // 5. Task Upvotes Table (Marketplace)
  db.exec(`
